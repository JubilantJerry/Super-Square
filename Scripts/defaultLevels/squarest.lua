...
function addPattern(mKey)
	...
	if mKey ==  0 then squareboxcap(50)
	elseif mKey ==  1 then squarerain(35)
	elseif mKey ==  2 then squaresolo(35)
	elseif mKey ==  3 then squareInverseBarrage(2,35)
	elseif mKey ==  4 then squarespiral(40)
	end
end

-- shuffle the keys, and then call them to add all the patterns
-- shuffling is better than randomizing - it guarantees all the patterns will be called
keys = { 0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3, 4, 4, 4}
keys = shuffle(keys)
index = 0
...
function onUpdate(mFrameTime)
	...
	if timerval < 0 then -- after 5 seconds flip direction
		...
		if stage == 2 then
			...
			oldrotspeed = getLevelValueFloat("rotation_speed")
			setLevelValueFloat("rotation_speed", getLevelValueFloat("rotation_speed") * 4)
		end
	end
	if stage == 2 then
...
function incrementsettings()
		...
		if getLevelValueFloat("speed_multiplier") > 2.4 then speedincrement = 0 end
		if getLevelValueFloat("rotation_speed") > 0.28 then rotincrement = 0 end
end