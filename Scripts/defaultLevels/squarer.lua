...
function addPattern(mKey)
	...
	if mKey ==  0 then squarestaggeredspiral(31)
	elseif mKey ==  1 then square1spin(41)
	elseif mKey ==  2 then squaresolo(31)
	elseif mKey ==  3 then squareInverseBarrage(2,31)
	elseif mKey ==  4 then squaremultisolo(31)
	elseif mKey ==  5 then square2spin(45)
	end
end

-- shuffle the keys, and then call them to add all the patterns
-- shuffling is better than randomizing - it guarantees all the patterns will be called
keys = { 0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5}
keys = shuffle(keys)
index = 0
...
function incrementsettings()
		...
		if getLevelValueFloat("speed_multiplier") > 1.8 then speedincrement = 0 end
		if getLevelValueFloat("rotation_speed") > 0.24 then rotincrement = 0 end
end